                ...
                render: (_value, item) => (
                    <div>
                        ...
                        <div className="panel__description">
                            ID:{' '}
                            <a href={`https://veles.finance/cabinet/backtests/${item.id}`} target="_blank"
            ...
            },
            {
                ...
                render: (_value, item) => (
                    <div>
                        ...
                        <div className="panel__description">в
                            день: {formatAmount(item.netQuotePerDay, item.quote)}</div>
                    </div>
                ),
            },
            {
        ...
        <section className="page">
            <header className="page__header">
                ...
                <p className="page__subtitle">
                    Журнал завершённых бэктестов с ключевыми метриками, пагинацией и возможностью выбирать результаты
            ...
            {!extensionReady && (
                <div className="banner banner--warning">
                    Расширение Veles Tools неактивно. Запустите интерфейс из меню расширения, чтобы загрузить список
                    бэктестов.
                </div>
            )}
                    ...
                    >
                      ...
                      {item.symbol}
                                ...
                                <span style={{
                                    marginLeft: 8,
                                    color: '#94a3b8'
                                }}>{formatAmount(item.profitQuote, item.quote)}</span>
                                {item.netQuote !== null && (
                                    <span style={{
                                        marginLeft: 8,
                                        color: '#94a3b8'
                                    }}>Net: {formatAmount(item.netQuote, item.quote)}</span>
                                )}
                            </li>
                        ))}
                    </ul>
            ...
            <div className="panel">
                <div className="panel__header">
                    <div>
                        ...
                        <p className="panel__description">
                            Соберите расширенную статистику выбранных бэктестов для анализа агрегированных метрик и
                            одновременных позиций.
                        </p>
                    </div>
                ...
                {botLimit !== null && totalSelected > 0 && (
                    <div className="aggregation-controls">
                        <label className="aggregation-controls__label" htmlFor="aggregation-bot-limit">
                            Блокировка по ботам
                            <InfoTooltip
                                text="Максимальное количество ботов, которые могут вести сделки одновременно. Если все слоты заняты, новые сделки будут пропущены до освобождения места."/>
                        </label>
                                    ...
                                    <div className="aggregation-summary">
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                ...
                                                <InfoTooltip
                                                    text="Количество бэктестов, включённых в расчёт агрегированных показателей."/>
                                            </div>
                                            <div
                                                className="aggregation-metric__value">{formatAggregationInteger(aggregationSummary.totalSelected)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Суммарный P&amp;L
                                                <InfoTooltip
                                                    text="Совокупный результат всех включённых бэктестов в выбранной валюте."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(aggregationSummary.totalPnl)}>{formatSignedAmount(aggregationSummary.totalPnl)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Avg P&amp;L / сделка
                                                <InfoTooltip
                                                    text="Средний результат одной сделки по всем включённым бэктестам."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(aggregationSummary.avgPnlPerDeal)}>{formatSignedAmount(aggregationSummary.avgPnlPerDeal)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Avg P&amp;L / бэктест
                                                <InfoTooltip
                                                    text="Средний итог на один бэктест в агрегированной выборке."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(aggregationSummary.avgPnlPerBacktest)}>{formatSignedAmount(aggregationSummary.avgPnlPerBacktest)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Средняя эффективность (Net/день)
                                                <InfoTooltip
                                                    text="Средний дневной результат по всем бэктестам, включённым в агрегированную статистику."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(aggregationSummary.avgNetPerDay)}>{formatSignedAmount(aggregationSummary.avgNetPerDay)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Сделки (P/L/Σ)
                                                <InfoTooltip
                                                    text="Количество сделок и распределение по прибыльным, убыточным и нейтральным операциям."/>
                                            </div>
                          ...
                          {' '}P:{formatAggregationInteger(aggregationSummary.totalProfits)} / L:{formatAggregationInteger(aggregationSummary.totalLosses)}
                                        ...
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                ...
                                                <InfoTooltip
                                                    text="Средняя продолжительность сделок в днях среди включённых бэктестов."/>
                                            </div>
                                            <div
                                                className="aggregation-metric__value">{formatAggregationValue(aggregationSummary.avgTradeDurationDays)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Средняя макс. просадка
                                                <InfoTooltip
                                                    text="Средняя максимальная просадка по каждому бэктесту; отрицательные значения указывают глубину падения."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(-Math.abs(aggregationSummary.avgMaxDrawdown))}>{formatAggregationValue(aggregationSummary.avgMaxDrawdown)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Макс. суммарная просадка
                                                <InfoTooltip
                                                    text="Максимальное падение совокупного портфеля, составленного из всех включённых бэктестов. Это максимальный зафиксированный убыток от вашего портфеля в какой то момент бектеста. Например при срабатывании нескольких стопов подряд."/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(-Math.abs(aggregationSummary.aggregateDrawdown))}>{formatAggregationValue(aggregationSummary.aggregateDrawdown)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Макс. суммарное МПУ
                                                <InfoTooltip
                                                    text="Максимальная одновременная просадка по всем бэктестам, учитывая пересечения сделок. Такое значение нереализованного P&L вы бы увидели в самый худший день за период бектестов"/>
                                            </div>
                                            <div
                                                className={resolveTrendClass(-Math.abs(aggregationSummary.aggregateMPU))}>{formatAggregationValue(aggregationSummary.aggregateMPU)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Макс. одновременно открытых
                                                <InfoTooltip
                                                    text="Пиковое количество одновременных позиций в любой день по всем бэктестам."/>
                                            </div>
                                            <div
                                                className="aggregation-metric__value">{formatAggregationInteger(aggregationSummary.maxConcurrent)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Среднее одновременно открытых
                                                <InfoTooltip
                                                    text="Среднее число одновременных позиций за весь период наблюдений."/>
                                            </div>
                                            <div
                                                className="aggregation-metric__value">{formatAggregationValue(aggregationSummary.avgConcurrent)}</div>
                                        </div>
                                        <div className="aggregation-metric">
                                            <div className="aggregation-metric__label">
                                                Дни без торговли
                                                <InfoTooltip
                                                    text="Количество дней без сделок в совокупном календаре выбранных бэктестов."/>
                                            </div>
                                            <div
                                                className="aggregation-metric__value">{formatAggregationValue(aggregationSummary.noTradeDays)}</div>
                                        </div>
                                    </div>
                                ),
                            },
                                    ...
                                    <div className="aggregation-equity">
                                        <div className="aggregation-equity__header">
                                            ...
                                            <p className="aggregation-equity__subtitle">
                                                Чёрная линия показывает изменение совокупного результата выбранных
                                                бэктестов.
                                            </p>
                                        </div>
                                        <div className="aggregation-equity__metrics">
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    Итоговый P&L
                                                    <InfoTooltip
                                                        text="Значение последней точки кривой портфельного P&L на основе выбранных бэктестов."/>
                                                </div>
                                                <div
                                                    className={portfolioFinalValue !== null ? resolveTrendClass(portfolioFinalValue) : 'aggregation-metric__value aggregation-metric__value--muted'}>
                                        ...
                                        <div className="aggregation-equity__chart">
                                            {portfolioEquitySeries && portfolioEquitySeries.points.length > 0 ? (
                                                ...
                                                <div className="aggregation-equity__empty">Нет данных для построения
                                                    графика.</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="aggregation-risk">
                                        <div className="aggregation-risk__header">
                                            ...
                                            <p className="aggregation-risk__subtitle">
                                                График показывает, как менялась совокупная потенциальная просадка по
                                                выбранным бэктестам.
                                            </p>
                                        </div>
                                        <div className="aggregation-risk__metrics">
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    ...
                                                    <InfoTooltip
                                                        text="Максимальная совокупная просадка (MPU) в любой момент времени по всем выбранным бэктестам."/>
                                                </div>
                                                <div
                                                    className={aggregateRiskPeak !== null ? resolveTrendClass(-Math.abs(aggregateRiskPeak)) : 'aggregation-metric__value aggregation-metric__value--muted'}>
                                        ...
                                        <div className="aggregation-risk__chart">
                                            {aggregateRiskSeries && aggregateRiskSeries.points.length > 0 ? (
                                                ...
                                                <div className="aggregation-risk__empty">Нет данных для построения
                                                    графика.</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="aggregation-limit">
                                        <div className="aggregation-limit__header">
                                            ...
                                            <p className="aggregation-limit__subtitle">
                                                Сравниваем итоговый P&amp;L, максимальную совокупную просадку и МПУ при
                                                разных ограничениях на количество ботов.
                                            </p>
                                        </div>
                                        <div className="aggregation-limit__chart">
                                            {limitImpactPoints.length > 0 ? (
                                                <LimitImpactChart points={limitImpactPoints}
                                                                  className="aggregation-limit__canvas"/>
                                            ) : (
                                                <div className="aggregation-limit__empty">Нет данных для построения
                                                    графика.</div>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="aggregation-concurrency">
                                        <div className="aggregation-concurrency__header">
                                            ...
                                            <p className="aggregation-concurrency__subtitle">
                                                Распределение дневных пиков помогает подобрать лимит одновременно
                                                открытых позиций.
                                            </p>
                                        </div>
                                        <div className="aggregation-concurrency__metrics">
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    Средний дневной пик
                                                    <InfoTooltip
                                                        text="Среднее значение дневного максимума активных позиций по совокупности бэктестов."/>
                                                </div>
                                                <div
                                                    className="aggregation-metric__value">{formatAggregationValue(dailyConcurrencyStats?.meanMax ?? 0)}</div>
                                            </div>
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    P75
                                                    <InfoTooltip
                                                        text="75-й перцентиль дневных максимумов активных позиций."/>
                                                </div>
                                                <div
                                                    className="aggregation-metric__value">{formatAggregationValue(dailyConcurrencyStats?.p75 ?? 0)}</div>
                                            </div>
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    P90
                                                    <InfoTooltip
                                                        text="90-й перцентиль дневных максимумов активных позиций."/>
                                                </div>
                                                <div
                                                    className="aggregation-metric__value">{formatAggregationValue(dailyConcurrencyStats?.p90 ?? 0)}</div>
                                            </div>
                                            <div className="aggregation-metric">
                                                <div className="aggregation-metric__label">
                                                    P95
                                                    <InfoTooltip
                                                        text="95-й перцентиль дневных максимумов активных позиций."/>
                                                </div>
                                                <div
                                                    className="aggregation-metric__value">{formatAggregationValue(dailyConcurrencyStats?.p95 ?? 0)}</div>
                                            </div>
                                        </div>
                                        <div className="aggregation-concurrency__chart">
                                            {dailyConcurrencyRecords.length > 0 ? (
                                                ...
                                                <div className="aggregation-concurrency__empty">Нет данных для
                                                    построения графика.</div>
                                            )}
                                        </div>
                                    </div>
                                ),
